                          ...
                          child: Constant.mapType == "inappmap"
                              ? Constant.selectedMapType == "osm"
                                  ...
                                  : GoogleMap(
                                      ...
                                      myLocationEnabled: controller.currentOrder.value.id != null && controller.currentOrder.value.status == Constant.driverPending ? false : true,
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        ...
                                        style:
                                            TextStyle(color: themeChange.getThem() ? AppThemeData.grey50 : AppThemeData.grey900, fontSize: 22, fontFamily: AppThemeData.semiBold),
                                      ),
                                      Text(
  ...
  buildOrderActionsCard(themeChange, HomeController controller) {
    ...
    if (controller.currentOrder.value.taxSetting != null) {
      for (var element in controller.currentOrder.value.taxSetting!) {
        taxAmount = taxAmount + Constant.calculateTax(amount: (subTotal - double.parse(controller.currentOrder.value.discount.toString())).toString(), taxModel: element);
      }
    }
            ...
            onTap: () {
              if (controller.currentOrder.value.status == Constant.orderShipped || controller.currentOrder.value.status == Constant.driverAccepted) {
                Get.to(const PickupOrderScreen(), arguments: {"orderModel": controller.currentOrder.value})?.then((v) async {
                  if (v == true && Constant.selectedMapType == 'osm') {
                    await controller.mapOsmController.removeMarker(controller.osmMarkers['Departure']!);
                    await controller.mapOsmController.removeMarker(controller.osmMarkers['Destination']!);
                    await controller.mapOsmController.clearAllRoads();
