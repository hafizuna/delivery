...
class HomeController extends GetxController {
  ...
  getCurrentOrder() async {
    if (Constant.singleOrderReceive == true) {
      if (driverModel.value.inProgressOrderID != null && driverModel.value.inProgressOrderID!.isNotEmpty) {
        FireStoreUtils.fireStore.collection(CollectionName.restaurantOrders).doc(driverModel.value.inProgressOrderID!.first.toString()).snapshots().listen(
          (event) async {
            if (event.exists) {
        ...
        FireStoreUtils.fireStore.collection(CollectionName.restaurantOrders).doc(driverModel.value.orderRequestData!.first.toString()).snapshots().listen(
          (event) async {
            if (event.exists) {
              currentOrder.value = OrderModel.fromJson(event.data()!);
              changeData();
        ...
        if (currentOrder.value.status == Constant.orderShipped) {
          ...
          PolylineResult result = await polylinePoints.value.getRouteBetweenCoordinates(
              ...
              request: PolylineRequest(
                  ...
                  destination: PointLatLng(currentOrder.value.vendor!.latitude ?? 0.0, currentOrder.value.vendor!.longitude ?? 0.0),
          ...
          PolylineResult result = await polylinePoints.value.getRouteBetweenCoordinates(
              ...
              request: PolylineRequest(
                  origin: PointLatLng(driverModel.value.location!.latitude ?? 0.0, driverModel.value.location!.longitude ?? 0.0),
                  destination: PointLatLng(currentOrder.value.address!.location!.latitude ?? 0.0, currentOrder.value.address!.location!.longitude ?? 0.0),
                  mode: TravelMode.driving));
  ...
  addPolyLine(List<LatLng> polylineCoordinates) {
    ...
    print("====polyLines=======${polyLines.value}");
    update();
      ...
      if (currentOrder.value.id != null) {
        if (currentOrder.value.status != Constant.driverPending) {
          if (currentOrder.value.status == Constant.orderShipped) {
            ...
            print(":::::::::::::${currentOrder.value.status}::::::::::::::::::");
            ...
            roadInfo.value = await mapOsmController.drawRoad(
              ...
              roadOption: RoadOption(
                roadBorderWidth: 15,
                roadBorderColor: AppThemeData.driverApp300,
                roadWidth: 15,
            ...
            print(":::::::::::::${currentOrder.value.status}::::::::::::::::::");
            GeoPoint sourceLocation = GeoPoint(latitude: driverModel.value.location!.latitude ?? 0.0, longitude: driverModel.value.location!.longitude ?? 0.0);

            GeoPoint destinationLocation =
                GeoPoint(latitude: currentOrder.value.address!.location!.latitude ?? 0.0, longitude: currentOrder.value.address!.location!.longitude ?? 0.0);
            setOsmMarker(departure: sourceLocation, destination: destinationLocation);
            roadInfo.value = await mapOsmController.drawRoad(
              sourceLocation,
              destinationLocation,
          ...
          GeoPoint destinationLocation = GeoPoint(latitude: currentOrder.value.vendor!.latitude ?? 0.0, longitude: currentOrder.value.vendor!.longitude ?? 0.0);
          await mapOsmController.removeLastRoad();
          setOsmMarker(departure: sourceLocation, destination: destinationLocation);
          roadInfo.value = await mapOsmController.drawRoad(
            sourceLocation,
